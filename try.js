...
;(function (window, undefined) {
  ...
  function initTry(userCfg) {
    loadScript('https://cdn.jsdelivr.net/npm/jquery@3.2.1/dist/jquery.min.js')
      ...
      .then(function () {
        return loadScript('https://cdn.jsdelivr.net/npm/swagger-ui-dist@3.25.1/swagger-ui-bundle.js')
      })
      .then(function () {
  ...
  function trySwagger(cfg) {
    initCss()
    {
      ...
      $('h1:eq(0)').after(
        $(
          '\n      <div class="'.concat(
            $('a[href*="swagger.json"]:eq(0)').attr('class'),
            ' btn setAuth">AUTHORIZE</div>\n    '
          )
        )
      )
    ...
    $('.tryBtn').click(function (event) {
      ...
      pos = Object.keys(pos).reduce(function (prev, cur, index) {
        ...
        return _objectSpread(
          _objectSpread({}, prev),
          {},
          _defineProperty({}, cur, typeof val === 'number' ? (val > 0 ? ''.concat(val, 'px') : undefined) : val)
        )
      }, {})
      ...
      $('.swaggerBox')
        ...
        .removeClass('hide')
        .addClass('show') // Synchronize the size of swaggerShadow to make it as big as swaggerBox
